...
- **Colaboración en tiempo real por LAN / VPN** (`Ctrl+Alt+L`): trabaja con más gente en el mismo proyecto. Un peer **aloja** una sesión (un servidor WebSocket en su máquina, sin dependencias — RFC 6455 implementado a mano) y comparte su **IP de LAN/VPN + puerto + token**; los demás se **unen** desde el panel. Sincroniza **chat de equipo** y **archivos** (al guardar se comparten con los demás; *last-write-wins*, **confinados al workspace** — se rechaza cualquier ruta que escape). La sesión está protegida por **token**. Nuevo `modules/collab-server.js` + panel de UI; verificado end-to-end (host + cliente) con tests de integración.
