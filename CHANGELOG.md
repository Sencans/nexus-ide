...
- **Colaboración en equipo por LAN / VPN con publicación revisada** (`Ctrl+Alt+L`): trabaja con más gente en un mismo proyecto. **Cada persona tiene su propia copia**; un peer **aloja** el proyecto central (servidor WebSocket sin dependencias, RFC 6455 a mano) y comparte su **IP + puerto + token**; los demás se **unen** (y reciben el proyecto central). Cuando alguien quiere actualizar el proyecto general, pulsa **«Publicar»**: antes de enviar se hace una **revisión automática del código** (sintaxis de JS con `node --check` y de JSON) y, si algo está roto, **se bloquea la publicación** para no romper el proyecto original. Si pasa, el central **la re-revisa** (doble red de seguridad), la aplica y la **propaga a todos**. Incluye chat de equipo. Escrituras **confinadas al workspace** y sesión protegida por **token**. Nuevo `modules/collab-server.js` + `reviewChangeset` en `agent-core.js` + panel de UI; verificado end-to-end (publicación válida se propaga, código roto se rechaza) con tests de integración. **Endurecido tras una auditoría adversarial multi-agente**: el central es la única autoridad (un peer no puede inyectar cambios a otros sin revisión), se propagan los **borrados** de archivos, no se pisan ediciones locales sin guardar, se excluyen **secretos** (`.env`, claves) y rutas peligrosas (`.git/`), y el servidor WebSocket reensambla frames fragmentados con tope anti-DoS.
- **Programador de tareas (cron)**: se pueden programar **tareas del agente en horarios cron** (Ajustes, JSON `[{id, cron, task, channel}]`) que se ejecutan solas y **entregan el resultado a un canal** (chat, Telegram o Discord). Útil para builds nocturnos, chequeos de PR o resúmenes periódicos, aprovechando los bots que ya existen. El *matcher* de cron de 5 campos (`* * * * *`, rangos, listas, pasos `*/n`) está en `agent-core.js` y testeado; una guarda por minuto evita ejecuciones duplicadas.
- **Learning loop (auto-skills)**: el agente puede **aprender habilidades reutilizables** a partir de las tareas que resuelve, con `[LEARN_SKILL: id | nombre | descripción | procedimiento]`. Se guardan en el sistema de habilidades existente (sin sobrescribir las del usuario) y quedan invocables con `[SKILL: id]` en sesiones futuras. El `id` se **sanea** para evitar path traversal al persistir. Parser testeado en `agent-core.js`.
- **Fallback entre proveedores**: se pueden configurar **modelos de respaldo** (Ajustes, separados por coma). Si el modelo principal falla (error de API, cuota agotada, red), el chat **reintenta automáticamente con el siguiente** hasta obtener respuesta, avisando del cambio. Orquestación testeada en `agent-core.js` (`runWithFallback`). Por defecto sin respaldos (comportamiento igual que antes).
